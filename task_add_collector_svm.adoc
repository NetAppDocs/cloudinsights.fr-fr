...
Gardez les points suivants à l'esprit pendant link:/cloudinsights/cs_restrict_user_access.html["Blocage de l'accès utilisateur"]:
...
....
Remarque : si un rôle REST est déjà ajouté--soit _arwrole_ soit _csrestrole_--il n'est pas nécessaire d'ajouter un deuxième rôle REST. Vous pouvez simplement ajouter les autorisations d'API comme dans l'exemple ci-dessous.

Exemple : _csrestrole_ est déjà présent. Il nous suffit donc d'activer la protection contre les ransomware et de donner des autorisations d'API au _csrestrole_ existant :
