...
Remarque : le niveau « utilisateur » par défaut de FOS n'est pas suffisamment puissant pour que Cloud Insights puisse afficher tous les aspects logiques d'un périphérique. Nous avons besoin d'un compte utilisateur avec le rôle « châssis » activé, ainsi que des autorisations sur toutes les structures virtuelles configurées sur un commutateur.

Voici un exemple de procédure permettant de créer un compte utilisateur avec un « privilège minimum » pour une utilisation de Cloud Insights dans une session SSH vers un périphérique FOS :
