...
* Dans les environnements Kubernetes mutualisés, les pannes peuvent se produire en raison de modifications mal configurées. Dans les environnements très dynamiques, le contrôle de l'infrastructure Cloud Insights peut ne pas être en mesure de suivre correctement tous les changements.
* L'analyse des changements permet d'y apporter une solution en fournissant une seule interface pour afficher et mettre en corrélation l'état des workloads et les modifications de configuration. Cela peut vous aider dans le dépannage des environnements Kubernetes dynamiques.
