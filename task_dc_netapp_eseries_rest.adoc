...
* Ce collecteur ne prend en charge que les baies de modèle E-Series dotées de *fonctionnalités d'API REST natives*. L'entreprise E-Series expédie une distribution d'API REST hors baie et installable pour les baies E-Series plus anciennes : ce collecteur ne prend pas en charge ce scénario. Les utilisateurs disposant de baies plus anciennes doivent continuer à utiliser Cloud Insights link:task_dc_na_eseries.html["API E-Series SANtricity"] collecteur.
