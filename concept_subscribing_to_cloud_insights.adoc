...
Les calculateurs d'abonnement vous donnent une estimation du coût Cloud Insights mensuel en fonction du nombre d'unités gérées dont vous aurez besoin. Les valeurs actuelles sont pré-remplies et vous pouvez ajuster ces valeurs pour vous aider à planifier la croissance future estimée. Vous pouvez ajuster les valeurs pour Infrastructure, Kubernetes, ou les deux.
