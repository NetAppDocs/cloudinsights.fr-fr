...
=== Prise en charge SAN iSCSI pour Kubernetes Network Map

Identifiez les dépendances du stockage sous-jacent Kubernetes pour NFS et maintenant iSCSI.
Accélérez la résolution des problèmes en obtenant une vue précise de l'activité du réseau de stockage de l'application Kubernetes ainsi que des metrics de stockage non délimités. Découvrez comment l'application Kubernetes utilise l'infrastructure de stockage pour le reporting de justification et de facturation interne.
...
Il est essentiel de garantir l'accessibilité de vos données, aussi nous avons réalisé l'exportation au format .CSV image:csv_export_icon.png["icône d'exportation .csv"] disponible pour toutes les requêtes de mesures, les widgets de tableau de bord et les pages d'arrivée d'objets, quel que soit le type de données (ressource ou intégration) que vous interrorez.
...
Avec Cloud Insights Premium Edition, les administrateurs peuvent activer *link:concept_user_roles.html["Authentification unique"]* (SSO) accès à Cloud Insights pour tous les utilisateurs de leur domaine d'entreprise, sans avoir à les inviter individuellement. Avec SSO activé, tous les utilisateurs disposant de la même adresse e-mail de domaine peuvent se connecter à Cloud Insights à l'aide de leurs informations d'identification d'entreprise.
