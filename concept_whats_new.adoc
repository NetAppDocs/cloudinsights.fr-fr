...
=== Icônes d'objet frais

Cloud Insights dispose désormais d'une iconographie d'objet mise à jour, réduisant ainsi la variance globale des icônes utilisées sur les différentes pages Cloud Insights et regroupant des objets similaires (par exemple, des objets liés au volume). Les icônes « à base de lettres » précédentes sont remplacées par des graphiques à base d'images, ce qui rend les écrans Cloud Insights plus conviviaux et plus accessibles.

Vous n'êtes pas sûr de la signification d'une icône ? Le tableau ci-dessous (également disponible sur la link:reference_object_icons.html["Référence de l'icône d'objet"] page) les explique tous.
