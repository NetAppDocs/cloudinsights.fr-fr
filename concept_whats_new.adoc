...
De nouvelles API de sécurité de la charge de travail sont disponibles pour gérer les collecteurs de données (ajout/mise à jour/suppression) et les agents. Accédez à *Admin > accès API > Documentation API* et sélectionnez le type d'API _Workload Security_ pour en savoir plus.
...
=== Meilleure sécurité des workloads

La sécurité des workloads comprend les améliorations suivantes :

* Les algorithmes de détection ont été améliorés pour réduire le nombre de fausses alertes positives.
* Les performances de l'interface utilisateur ont été améliorées lors du chargement, de l'affichage et du défilement des pages Forensics.
...
* *Accès refusé* : la sécurité de la charge de travail s'intègre à ONTAP pour recevoir des événements « accès refusé » et fournir une couche supplémentaire d'analyses et de réponses automatiques.
