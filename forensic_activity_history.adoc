...
<<<<<<<< TÊTE * *réduction du bruit* pour filtrer les fichiers créés au cours des 2 dernières heures par l'utilisateur. Il est également utilisé pour filtrer les fichiers temporaires (par exemple, les fichiers .tmp) auxquels l'utilisateur a accès.
...
== Tri des données d'historique d'activité médico-légale
...
|===
== chemin / chemin d'origine recherche
