...
toc::[]
L'opérateur Kubernetes et les collecteurs de données sont téléchargés depuis le registre Docker de Cloud Insights. Une fois installé, l'opérateur gère ensuite tous les collecteurs compatibles avec l'opérateur déployés dans les nœuds du cluster Kubernetes pour acquérir des données, y compris la gestion du cycle de vie de ces collecteurs. Après cette chaîne, les données sont acquises auprès des collecteurs et envoyées à Cloud Insights.
...
IMPORTANT: Lire le link:pre-requisites_for_k8s_operator.html["*Avant l'installation ou la mise à niveau*"] Documentation requise avant l'installation ou la mise à niveau de l'opérateur de surveillance NetApp Kubernetes.
